...
class ClassroomService {
  ...
  Future<void> joinRoom({
    ...
    _socket!.on('participant-joined', (data) {
      ...
      if (data['role'] == 'mentor' || data['role'] == 'admin') {
        onMentorJoined?.call(data['socketId'], data['userName'],
            role: data['role']);
      }
      _startPeerConnection(data['socketId'], userName,
          isOffer: role == ClassroomRole.mentor);
    });
  ...
  Future<void> _startPeerConnection(String targetId, String localName,
    ...
    if (isOffer) {
      ...
      _socket!.emit('offer',
          {'target': targetId, 'offer': offer.toMap(), 'fromName': localName});
    }
  }

  Future<void> _handleOffer(Map<String, dynamic> data, String localName) async {
    if (_peerConnection == null)
      await _startPeerConnection(data['from'], localName, isOffer: false);
