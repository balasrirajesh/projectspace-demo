...
class _UserManagementPageState extends State<UserManagementPage> {
        ...
        Expanded(
          child: Container(
            ...
            child: TextField(
              ...
              onChanged: (val) => admin.fetchUsers(search: val, role: _selectedRole == 'all' ? null : _selectedRole),
  ...
  Widget _filterChip(String label, String value, AdminProvider admin) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        admin.fetchUsers(role: value == 'all' ? null : value, search: _searchController.text);
      },
      child: Container(
  ...
  Widget _buildUserTable(AdminProvider admin) {
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        rows: admin.users.map((user) => _buildUserRow(context, admin, user)).toList(),
      ),
    );
  }

  DataRow _buildUserRow(BuildContext context, AdminProvider admin, dynamic user) {
      ...
      cells: [
          ...
          Row(
            children: [
              CircleAvatar(
                ...
                child: Text(user['name']?[0] ?? '?', style: const TextStyle(color: Colors.indigo, fontWeight: FontWeight.bold)),
              ),
              ...
              Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(user['name'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                  Text(user['email'] ?? '', style: TextStyle(color: Colors.blueGrey[400], fontSize: 12)),
                ],
              ),
            ],
          ),
        ...
        DataCell(
          Container(
            ...
            decoration: BoxDecoration(
              color: (user['role'] == 'student' ? Colors.blue : Colors.purple).withOpacity(0.08),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Text(
              user['role']?.toUpperCase() ?? '',
              style: TextStyle(color: user['role'] == 'student' ? Colors.blue : Colors.purple, fontSize: 10, fontWeight: FontWeight.w900),
            ),
          ),
        ),
        DataCell(Text(user['collegeName'] ?? 'Not Specified', style: const TextStyle(fontSize: 13))),
        DataCell(
          Container(
          ...
          Row(
            children: [
              IconButton(
                ...
                icon: const Icon(Icons.edit_note_rounded, color: Colors.blueGrey, size: 20)
              ),
    ...
    return PopupMenuButton(
      ...
      itemBuilder: (context) => [
        if (user['status'] != 'verified')
          const PopupMenuItem(value: 'verified', child: Text("Approve & Verify")),
        const PopupMenuItem(value: 'blocked', child: Text("Block User", style: TextStyle(color: Colors.red))),
  ...
  void _showUserDetail(BuildContext context, AdminProvider admin, dynamic user) {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  radius: 30,
                  backgroundColor: Colors.indigo.withOpacity(0.1),
                  child: Text(user['name']?[0] ?? '?', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.indigo)),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(user['name'] ?? 'User Profile', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ...
            const SizedBox(height: 32),
            const Text("Administrative Controls", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            ...
            ListTile(
              ...
              onTap: () {
                final newRole = user['role'] == 'student' ? 'mentor' : 'student';
                admin.updateUserStatus(user['id'], newRole); // We use status update endpoint for role too in this demo
                Navigator.pop(context);
              },
            ),
            const Spacer(),
            Text("User ID: ${user['id']}", style: const TextStyle(color: Colors.grey, fontSize: 10)),
          ],
        ),
      ),
    );
