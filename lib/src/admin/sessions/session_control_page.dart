...
class _SessionControlPageState extends State<SessionControlPage> {
                    ...
                    return SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (!admin.isLoading || admin.activeSessionsList.isNotEmpty)
  ...
  Widget _buildErrorState(AdminProvider admin) {
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.wifi_off_rounded, size: 48, color: Colors.orange.withOpacity(0.5)),
          const SizedBox(height: 16),
          const Text("Connection Interrupted", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 8),
          Text("We're having trouble reaching the signaling server. Live monitor is currently offline.", 
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Expanded(
            child: Text(
              "Sync failed: check server status", 
              style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)
            ),
          ),
          TextButton(
            onPressed: () => admin.fetchActiveSessions(),
            child: const Text("RETRY", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold, fontSize: 12)),
          ),
        ],
      ),
  ...
  Widget _buildLiveBadge(int count) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: count > 0 ? Colors.red.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: count > 0 ? Colors.red.withOpacity(0.2) : Colors.grey.withOpacity(0.2)),
      ),
      child: Row(
      ...
      builder: (context, constraints) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: Colors.indigo.withOpacity(0.1), blurRadius: 40, offset: const Offset(0, 20)),
            ],
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildStartBtn() {
    return ElevatedButton.icon(
      ...
      label: const Text("Start Official Session", style: TextStyle(fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget _buildSessionsList(AdminProvider admin) {
    if (admin.activeSessionsList.isEmpty) {
      return Container(
        ...
        child: Column(
          children: [
            Icon(Icons.monitor_heart_rounded, size: 64, color: Colors.grey[200]),
            const SizedBox(height: 16),
            Text("No active sessions currently monitored", style: TextStyle(color: Colors.blueGrey[200], fontWeight: FontWeight.w500)),
          ],
        ),
      );
    }
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => InteractiveClassroomPage(roomId: session['id'])),
            );
          },
          child: Padding(
            padding: const EdgeInsets.all(24),
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Icon(Icons.person_pin_rounded, size: 14, color: Colors.blueGrey[300]),
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: const FittedBox(child: Text("Superior Stop", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                    ),
                  ],
                ),
              ],
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 4),
          Text(count.toString(), style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.bold, fontSize: 11)),
        ],
      ),
    );
  }
  ...
  Widget _buildStartTime(String? startTime) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("STARTED", style: TextStyle(color: Colors.blueGrey[200], fontSize: 9, fontWeight: FontWeight.bold)),
        Text(
  ...
  void _confirmTerminate(AdminProvider admin, String roomId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("You are about to forcefully disconnect all participants and terminate this session. This action cannot be undone. Proceed?"),
        actions: [
            ...
            onPressed: () async {
              ...
              if (success && mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text("Session terminated successfully."), backgroundColor: Colors.red),
                );
              }
            },
  ...
  void _startAdminSession() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.rocket_launch_rounded, size: 48, color: Colors.indigo),
            const SizedBox(height: 16),
            const Text("Official Session Dashboard", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text("You are now initializing an official Faculty-led classroom. This session will be broadcasted to all eligible students.", textAlign: TextAlign.center),
            const Spacer(),
            ElevatedButton(
              onPressed: () {
                final roomId = "FACULTY-${math.Random().nextInt(9999).toString().padLeft(4, '0')}";
                Navigator.pop(context); // Close sheet
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => InteractiveClassroomPage(roomId: roomId)),
                );
              },
              style: ElevatedButton.styleFrom(
