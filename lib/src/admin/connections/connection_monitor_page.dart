...
class _ConnectionMonitorPageState extends State<ConnectionMonitorPage> {
  ...
  Widget _buildConnectionCard(BuildContext context, AdminProvider admin, dynamic conn) {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  conn['topics'] != null ? "Topics: ${(conn['topics'] as List).join(', ')}" : "Mentorship Request", 
  ...
  Widget _buildActionMenu(BuildContext context, AdminProvider admin, dynamic conn) {
    return PopupMenuButton(
      icon: const Icon(Icons.more_horiz_rounded, color: Colors.blueGrey),
      onSelected: (val) async {
        final success = await admin.updateConnectionStatus(conn['id'], val.toString());
        if (success && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Connection status updated to $val"), backgroundColor: Colors.indigo),
          );
        }
      },
      itemBuilder: (context) => [
        const PopupMenuItem(value: 'accepted', child: Text("Force Connect (Accept)")),
        const PopupMenuItem(value: 'rejected', child: Text("Force Disconnect (Reject)", style: TextStyle(color: Colors.red))),
