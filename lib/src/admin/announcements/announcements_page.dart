...
class _AnnouncementsPageState extends State<AnnouncementsPage> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text("Broadcast Announcement", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text("Send an urgent notification to your college network", style: TextStyle(color: Colors.grey)),
  ...
  void _sendBroadcast() async {
    ...
    final success = await context.read<AdminProvider>().broadcastAnnouncement(
      title, 
      message, 
      _targetGroup
    );
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Failed to send broadcast. Check connection.")),
      );
    }
  }
}
