...
class _MentorshipRequestFormState extends State<MentorshipRequestForm> {
  ...
  void _submit() {
    if (_reasonController.text.isEmpty || _selectedTopics.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please provide a reason and select at least one topic.")),
      );
      return;
    }
  ...
  void _onReasonChanged(String text) {
    ...
    if (text.toLowerCase().contains("resume") || text.toLowerCase().contains("job")) {
      if (!_selectedTopics.contains("Resume & Portfolio Review")) {
        setState(() => _selectedTopics.add("Resume & Portfolio Review"));
      }
    }
    if (text.toLowerCase().contains("flutter") || text.toLowerCase().contains("app")) {
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: "E.g., I'm preparing for my first Flutter interview...",
                filled: true,
