...
class _ProfileSetupPageState extends State<ProfileSetupPage> {
  ...
  final List<String> _suggestedSkills = ["Flutter", "Dart", "System Design", "Backend", "React", "Cloud", "UI/UX", "Data Science", "Python", "Java", "Node.js", "AWS", "Docker", "Kubernetes", "Machine Learning", "Product Management"];
  ...
  void _previousPage() {
    if (_currentPage > 0) {
      _pageController.previousPage(duration: 600.ms, curve: Curves.easeInOutCubic);
    }
  }
          ...
          Stack(
            children: [
              ...
              AnimatedContainer(
                duration: 400.ms,
                height: 6,
                width: MediaQuery.of(context).size.width * ((_currentPage + 1) / _totalSteps) - 48,
                decoration: BoxDecoration(
  ...
  Widget _buildPersonalStep() {
    return _buildStepLayout(
      ...
      children: [
        _buildAnimatedField(
          index: 0,
          child: _buildInputField("Full Name", _fullNameController, Icons.person_outline_rounded),
        ),
        _buildAnimatedField(
          index: 1,
          child: _buildInputField("Phone Number", _phoneController, Icons.phone_android_rounded, keyboardType: TextInputType.phone),
        ),
        _buildAnimatedField(
          index: 2,
          child: _buildInputField("Bio / Mentoring Philosophy", _bioController, Icons.description_outlined, maxLines: 3),
        ),
      ],
    );
  }
  ...
  Widget _buildProfessionalStep() {
    return _buildStepLayout(
      ...
      children: [
        ...
        _buildAnimatedField(index: 2, child: _buildInputField("Current Company", _companyController, Icons.business_rounded)),
        _buildAnimatedField(index: 3, child: _buildInputField("Current Job Title", _jobController, Icons.work_outline_rounded)),
        ...
        _buildAnimatedField(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text("Expertise & Skills", style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 18)),
                  const Spacer(),
                  Text("${_selectedSkills.length} selected", style: TextStyle(color: AppColors.primary, fontSize: 12, fontWeight: FontWeight.w600)),
                ],
              ),
  ...
  Widget _buildSocialStep() {
    return _buildStepLayout(
      ...
      children: [
        _buildAnimatedField(
          index: 0,
          child: _buildInputField("LinkedIn Profile URL", _linkedInController, Icons.link_rounded),
        ),
        _buildAnimatedField(
          index: 1,
          child: _buildInputField("GitHub / ResearchGate URL", _githubController, Icons.code_rounded),
        ),
        _buildAnimatedField(
          index: 2,
          child: _buildInputField("Personal Portfolio / Official Webpage", _portfolioController, Icons.language_rounded),
        ),
        const SizedBox(height: 24),
        Container(
          padding: const EdgeInsets.all(16),
          ...
          _buildAnimatedField(
            ...
            child: GestureDetector(
              onTap: _isUploading ? null : () async {
                ...
                await Future.delayed(const Duration(seconds: 1)); // Simulate picker
                setState(() {
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      _isUploading ? "Uploading..." : (_selectedFileName ?? "Tap to upload document"), 
                      style: GoogleFonts.poppins(
                        fontWeight: FontWeight.w600, 
                        fontSize: 14,
                        color: _selectedFileName != null ? Colors.green : AppColors.textPrimary
                      )
                    ),
                    if (_selectedFileName == null && !_isUploading)
                      const Text("PDF, JPG, PNG (Max 5MB)", style: TextStyle(fontSize: 11, color: AppColors.textLight)),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildStepLayout({required String title, required String subtitle, required List<Widget> children}) {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold, height: 1.2)).animate().fadeIn().slideX(begin: -0.1),
          const SizedBox(height: 8),
          Text(subtitle, style: GoogleFonts.inter(fontSize: 14, color: AppColors.textSecondary)).animate().fadeIn(delay: 200.ms).slideX(begin: -0.1),
  ...
  Widget _buildAnimatedField({required int index, required Widget child}) {
    return Padding(
      ...
      child: child.animate().fadeIn(delay: (400 + (index * 100)).ms).slideY(begin: 0.1),
    );
  }
  ...
  Widget _buildDropdownField(String label, TextEditingController controller, IconData icon, List<String> items) {
  ...
  Widget _buildVerificationCard(String title, String desc, IconData icon, {bool isSelected = false}) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: isSelected ? AppColors.primary : Colors.grey[200]!, width: isSelected ? 2 : 1.5),
      ),
      child: Row(
        children: [
        ...
        children: [
          ...
          Expanded(
            ...
            child: ElevatedButton(
              onPressed: _nextPage,
              child: Text(_currentPage == _totalSteps - 1 ? "Complete Setup" : "Continue"),
            ),
          ),
        ],
      ),
...
class _ScanningAnimationState extends State<_ScanningAnimation> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(
        children: [
          const Icon(Icons.verified_user_outlined, size: 100, color: Colors.grey),
          AnimatedBuilder(
