      ...
      if (mounted) {
        setState(() {
          ...
          if (e == 'CAMERA_PERMISSION_DENIED') {
            _errorMessage = "Failed to start classroom: Unable to getUserMedia: NotAllowedError: Permission denied";
          } else if (e == 'SIGNALING_TIMEOUT' || e.toString().contains('signaling')) {
            _errorMessage = "Signaling server unreachable at ${AuthProvider.getSignalingUrl()}";
          } else {
      ...
      child: const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 20),
            Text("Establishing secure connection...", style: TextStyle(color: Colors.white70)),
          ],
        ),
      ),
    );
                  ...
                  if (_errorMessage!.contains("Permission denied"))
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () {}, // For web, might need to show instruction dialog
                        icon: const Icon(Icons.settings, color: Colors.white),
                        label: const Text("How to allow", style: TextStyle(color: Colors.white)),
