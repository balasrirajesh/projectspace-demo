...
class _MenteesPageState extends State<MenteesPage> with SingleTickerProviderStateMixin {
  late TabController _tabController;

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this, initialIndex: widget.initialShowActive ? 0 : 1);
  }
    ...
    return Consumer<MentorshipProvider>(
      builder: (context, provider, child) {
        final mentees = isActive 
            ...
            : provider.allRequests.where((r) => r.status == MentorshipStatus.ended).toList();
        ...
        if (mentees.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(isActive ? Icons.people_outline_rounded : Icons.history_rounded, 
                  size: 80, 
                ...
                Text(isActive ? "Accept a request to start mentoring!" : "Your past mentorships will appear here.", 
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            spacing: 6,
                            children: student.skills.take(2).map((s) => Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                              decoration: BoxDecoration(
                                color: AppColors.accent.withOpacity(0.05),
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Text(s, style: const TextStyle(fontSize: 10, color: AppColors.accent, fontWeight: FontWeight.w600)),
                            )).toList(),
                          ),
                        ],
                      ),
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Connecting to secure chat session..."))
                          );
                        },
                      ),
                  ],
