...
class ClassroomService {
    ...
    try {
      ...
      if (useMedia) {
        try {
          ...
          dev.log(
              '⚠️ [RTC] Media Denied: $mediaErr. Continuing with signaling only.');
        }
      }
      ...
      _socket!.emit('join-room', {
        'roomId': _roomId,
        'role': AuthProvider.isUserAdmin
            ? 'admin'
            : (_role == ClassroomRole.mentor ? 'mentor' : 'student'),
        'userName': userName,
        'title': title ?? roomId,
      });
      ...
      Future.delayed(
          const Duration(milliseconds: 100), () => onConnected?.call());
    } catch (e) {
  ...
  void _registerBasicEvents(String serverUrl, String userName) {
    ...
    _socket!.onError((err) {
      ...
      if (errStr.contains('503')) {
        onError?.call(
            'Production Server is currently unavailable (503). It might be scaling up or undergoing maintenance.');
        return;
      }

      if (!errStr.contains('TransportError') &&
          !errStr.contains('xhr poll error')) {
      ...
      participantMap.forEach((id, metadata) {
        if (id != _socket!.id) {
          ...
          if (role == 'mentor' || role == 'admin') {
            onMentorJoined?.call(id, metadata['userName'] ?? 'Host',
                role: role);
          }
    ...
    _socket!.on('participant-joined', (data) {
      ...
      participants[id] = {'role': data['role'], 'userName': data['userName']};
    ...
    _socket!
        .on('ice-candidate', (data) async => await _handleIceCandidate(data));

    // Global Events
    _socket!.on('mentor-left',
        (_) => onError?.call('The educational session has ended.'));
    _socket!.on(
        'new-message',
        (data) =>
            onChatMessage?.call(data['userName'] ?? 'Unknown', data['text']));
    _socket!.on(
        'user-raised-hand',
        (data) => onHandRaised?.call(
            data['userName'] ?? 'Someone', data['isRaised'] ?? true));
    _socket!.on(
        'room-list', (data) => onRoomListUpdate?.call(data as List<dynamic>));
    _socket!.on(
        'new-announcement',
        (data) => onAnnouncementReceived
            ?.call(Map<String, dynamic>.from(data as Map)));
  ...
  Future<RTCPeerConnection> _createPeerConnection(
      String remoteId, String localName) async {
  ...
  Future<void> _createFallbackStream(
      String remoteId, MediaStreamTrack track) async {
  ...
  // Messaging Helpers
  void sendMessage(String text, String userName) => _socket?.emit(
      'send-message', {'text': text, 'roomId': _roomId, 'userName': userName});
  ...
  void raiseHand(String userName, bool isRaised) => _socket?.emit('raise-hand',
      {'roomId': _roomId, 'userName': userName, 'isRaised': isRaised});
